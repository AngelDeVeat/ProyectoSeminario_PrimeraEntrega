...
public class PersonaAltaUseCase
{
    ...
    public void Alta(Persona persona, int IdUsuario)
    {
        ...
        if (Validador_Persona.isUnique_DNI(persona.DNI, _ipersona))
            throw new DuplicadoException("Validacion fallada debido a que se intento crear una instancia de la clase Persona con un DNI ya existente");
        if (Validador_Persona.isUnique_Email(persona.Email, _ipersona))
            throw new DuplicadoException("Validacion fallada debido a que se intento crear una instancia de la clase Persona con un Email ya existente");
